...
# box: the marketing landing site and the CI config. The running app serves
# src/public, never landing/, so these are just dead weight here. (guides/ is
# gitignored and never cloned.) Runs every time — a later `git reset --hard`
# during update restores them, and this removes them again. Idempotent.
rm -rf landing .github 2>/dev/null || true

...
// sends one daily digest at the configured hour. Reads live settings, so
// toggling email or changing thresholds from the dashboard applies immediately.
// In-memory state only — a restart simply re-arms everything, which is fine.
...
async function tick(): Promise<void> {
  if (!emailEnabled()) return; // off, or toggled off from the dashboard
  try {
...
// Always runs — the tick early-exits when email is off, so enabling it later
// from the dashboard takes effect without a restart.
export function startWatcher(): void {
  setTimeout(tick, FIRST_CHECK_DELAY_MS);
  setInterval(tick, CHECK_INTERVAL_MS);
  const al = getAlerts(), em = getEmail();
  const digest = al.digestHour >= 0 ? `${String(al.digestHour).padStart(2, "0")}:00` : "off";
  console.log(`  Email: ${emailEnabled() ? em.to : "off"}  |  alerts: disk≥${al.diskPct}% mem≥${al.memPct}%, daily report ${digest}`);
}
